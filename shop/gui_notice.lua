...
function notice(application, timeout, message, callback, button_active, button_delay)
    ...
    local panel2          = application:addChild(GUI.panel(1, math.floor(bufferHeight / 2 - height / 2), application.width, height + offset * 2, 0x1D1D1D))
    local image_ch        = application:addChild(GUI.image(x, math.floor(bufferHeight / 2 - height / 2) + y - 1, sign))
    local textbox         = application:addChild(GUI.textBox(x + image.getWidth(sign) + 2, math.floor(bufferHeight / 2 - height / 2) + y, textWidth, #lines, 0x1D1D1D, 0xE1E1E1, lines, 1, 0, 0))
    textbox.eventHandler  = nil
    local label_countdown = application:addChild(GUI.label(panel2.x + panel2.width - 2, panel2.y + 1, 3, 1, 0x33DB40, ""))
    
    local buttonWidth     = 10
    local button          = application:addChild(GUI.roundedButton(x + image.getWidth(sign) + textWidth - buttonWidth + 2, panel2.y + height + offset, buttonWidth, 1, 0x3366CC, 0xE1E1E1, 0xE1E1E1, 0x3366CC, "OK"))
